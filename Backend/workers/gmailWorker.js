...
const gmailWorkerHandler = async (redisConnection) => {
    const gmailWorker = new Worker("gmail-notifications", async (job) => {
        ...
        const { tokens, tags } = await getUserDetailsAndTaginputs(emailAddress);
        if (!tokens || !tags) {
        ...
        const result = await processIncomingEmailsWithHistory(oAuth2Client, tags, emailAddress, historyId);
